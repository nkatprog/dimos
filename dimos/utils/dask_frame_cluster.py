...
class SourceActor:
    ...
    def __init__(self, shape, dtype="uint8", prefer="auto", device_id=0):
        ...
        self.channel = make_frame_channel(self.shape, dtype=self.dtype, prefer=prefer, device=device_id)
...
class ModelActor:
    ...
    def __init__(self, desc, model_name, fps):
        ...
        self.proc = MultiRateProcessor(channel=self.slot,
...
def make_cluster(mode: str):
    ...
    if mode == "cuda":
        try:
            ...
            print("[warn] CUDA cluster requested but dask-cuda/UCX not available; falling back to CPU LocalCluster.")
    # CPU cluster
...
def main(mode: str):
    ...
    src_fut = client.submit(SourceActor, shape, "uint8", prefer, 0, actor=True, workers=[workers[0]])
    src = src_fut.result()
