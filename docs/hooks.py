...
@contextmanager
def _managed_process(*args, name: str = "process", **kwargs):
    ...
    Iteratively kills children since processes like Dask workers may spawn new
    children during shutdown. Uses wait_procs() to ensure children are actually
    dead before proceeding.

    After killing, captures and prints any stdout/stderr for debugging.

    Inspired by https://gist.github.com/jizhilong/6687481#gistcomment-3057122
    """
    ...
    try:
        ...
        try:
            ...
            # Iteratively kill children - Dask may spawn more during shutdown
            for _ in range(3):
            ...
            pass  # Continue anyway - proc.wait() below will handle it

        proc.wait()
        # Now safe to read pipes - process is dead
        _print_process_output(name, proc)


def _print_process_output(name: str, proc: subprocess.Popen) -> None:
    """Print captured stdout/stderr from a process for debugging."""
...
def _export_notebook(source: Path, output: Path, timeout: int = 180) -> bool:
    ...
    cmd = ["marimo", "export", "html", str(source), "-o", str(output), "--force", "--no-sandbox"]
    ...
    with _managed_process(
        ...
        while time.time() - start < timeout:
            ...
            if proc.poll() is not None:
                if proc.returncode == 0:
                    ...
                    # Output printed by context manager's finally block
                    return True
                else:
                    print(" failed!")
            ...
            if output.exists():
                ...
                if current_size > min_file_size and current_size == last_size:
                    ...
                    if stable_count >= stable_threshold:
                        # File write is complete - context manager kills process & prints output
        ...
        # Timeout reached - context manager kills process & prints output
        elapsed = time.time() - start
